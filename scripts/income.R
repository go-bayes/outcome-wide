...
# read data
df <- readRDS(here::here("data", "amydf.rds"))



# need to transform certain vars
# specific cases
hist(as.numeric(df$BornNZ))
hist(df$Hours.Exercise)
hist(df$Religion.Church)
hist(df$Household.INC) # not useful
...
abline(v = mean(df$HLTH.BMI, na.rm=TRUE), col = "blue", lwd=3)
table(df$EthnicCats)
...
adf <-
  ...
  select(
    ...
    Hours.Work,
    Hours.Exercise,
    # Emp.WorkLifeBalance,  not measured 2018
    LIFEMEANING,
    LIFESAT,
    PWI,
    NWI,
  ...
  dplyr::select(-c(Religion.Church, EthCat, Respect.Self_lead2,
                   CharityDonate, Household.INC, Alcohol.Intensity,org2018,hold19,hold18,
                   Emp.WorkLifeBalance,
                   YearMeasured, org2019, Hours.Exercise, Employed, Hours.Exercise_lead2))%>%  # no info in inc_prop
  # dplyr::mutate(across(!c(Id,Wave), ~ scale(.x)))%>%
  arrange(Id,Wave) %>%
  data.frame() %>%
