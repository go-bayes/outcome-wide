...
dat_prep  <- dat_new %>%
  ...
  dplyr::filter(!is.na(KESSLER6sum)) %>% # min income
  dplyr::filter(!is.na(KESSLER6sum_lead1)) %>% # min income
  dplyr::select(
    -c(
      Religion.Church,
...
skim(dat_prep) |>
  arrange(n_missing)
# save data
saveh(dat_prep, "outcomewide-distress-dat_prep")

# read if needed
dat_prep <- readh("outcomewide-distress-dat_prep")
...
# save
saveh(ppm_mice, "outcomewide-distress-ppm_mice")

# read
ppm_mice <- readh( "outcomewide-distress-ppm_mice" )
...
#save
saveh(ml,"outcomewide-distress-ml")
saveh(mf,"outcomewide-distress-mf")
...
saveh(no_miss_model,"outcomewide-distress-no-miss-model")
