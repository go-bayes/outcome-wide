...
dat$EQUAKE.Affected.Earthquake
...
dat %>%
  group_by(Wave) %>%
  summarise(mean(Earthquake.Anxiety, na.rm = TRUE),
            sd(Earthquake.Anxiety,  na.rm = TRUE))
...
#  dplyr::mutate(Church_lead1 = lead(Church, n = 1)) %>%
  ...
  dplyr::filter(!is.na(Earthquake.Anxiety)) %>%
  dplyr::filter(!is.na(Earthquake.Anxiety_lead1)) %>%
  dplyr::filter(!is.na(Env.ClimateChgConcern)) %>%
  dplyr::filter(!is.na(Env.ClimateChgConcern_lead1)) %>%
  dplyr::filter(!is.na(Your.Future.Security)) %>%
  dplyr::filter(!is.na(Your.Future.Security_lead1)) %>%
  dplyr::filter(!is.na(Your.Health)) %>%
  dplyr::filter(!is.na(Your.Health_lead1)) %>%
...
# save
saveh(cr_mice, "cause_belief_mice")

# read
cr_mice <- readh("cause_belief_mice")
# checks
outlist2 <-
...
saveh(ml, "l_belief_cause_all")
saveh(mf, "f_belief_cause_all")
