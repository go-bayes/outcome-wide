...
length(unique(dat_prep$Id)) # 1367
...
#     Wave
#   ) |>
#   dplyr::mutate(pets = as.numeric(Pets),
#                 pets_coded = as.numeric(Pets.coded.catdog_factor)) |>
#   dplyr::mutate(pet_baseline = ifelse(
#     (Wave == 2015 &
#        pets == 2),
#     1,
#     0
#   )) |>
#   dplyr::filter(pet_baseline == 1 | Wave == 2019) |>
#   droplevels() |>
#   arrange(Wave, Id)
#
# table(lost$pet_baseline)
#
# length(unique(lost$Id))
#
# msm::statetable.msm(round(pets, 0), Id, data = lost) |>
#   kbl() |>
#   kable_paper(full_width = F)
...
corr_powerdep <- cbind.data.frame(
  dat_prep$kessler_hopeless,
...
saveRDS(
  ...
  here::here(
    ...
    "outcomewide-pets-ppm_mice_lost"
  )
)
...
saveRDS(
  ...
  here::here(
    "/Users/joseph/v-project\ Dropbox/Joseph\ Bulbulia/outcomewide/pets",
    "outcomewide-pets-no-miss-model"
  )
)
