...
df <- readRDS(here::here( "/Users/joseph/v-project\ Dropbox/Joseph\ Bulbulia/outcomewide/pets", "outcomewide-pets-ml"))
mf <- readRDS( here::here( "/Users/joseph/v-project\ Dropbox/Joseph\ Bulbulia/outcomewide/pets", "outcomewide-pets-mf"))
...
gcomp_forestplot_pets <- gcomp_forestplot(out_pets, title = "Outcomewide Pets (difference scale), N = 8675", ylim = c(-.5,.5), xlab = "Incidence Pet (SD)")
...
k_lmer <- lme4::lmer(KESSLER6sum_z ~ Wave * pets +  AGREEABLENESS_z +
                     CONSCIENTIOUSNESS_z +
                     EXTRAVERSION_z +
                     HONESTY_HUMILITY_z +
                     NEUROTICISM_z +
...
k_ml<- plot(
  ml_tab<-  ggeffects::ggpredict(k_lmer, terms = c( "Wave", "pets")
...
ggsave(
  ...
  height = 8,
  units = "in",
  filename = "bad_multi-level-model.jpg",
  device = 'jpeg',
  limitsize = FALSE,
  dpi = 1200
)
