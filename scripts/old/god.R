...
c_df <- df %>%
  ...
  dplyr::mutate(god_lead1 = lead(Believe.God, n = 1)) %>%
  dplyr::mutate(spirit_lead1 = lead(Believe.Spirit, n = 1)) %>%
  dplyr::filter(Wave == 2018) %>%
  dplyr::filter(!is.na(Believe.God)) %>%
  dplyr::filter(!is.na(god_lead1)) %>%  #needed for the intervention
  dplyr::filter(!is.na(Believe.Spirit)) %>%
  dplyr::filter(!is.na(spirit_lead1)) %>%  #needed for the intervention
  dplyr::select(
    -c(
      Religion.Church,
      EthCat,
...
# impute
out_god <- mice::mice(g_mice,
                      #meth = "rfcont",
                      #pred = pred,
                      seed = 0, m = 10)

# save
saveh(out_god, "out_god")

# read
out_god <- readh("out_god")
warnings()

outlist2 <-
  row.names(out_god)[out_god$outflux < 0.5]
length(outlist2)

head(out_god$loggedEvents, 10)
...
# data warangling
long <- mice::complete(out_god, "long", inc = TRUE)
...
long2 <- long %>%
  ...
  dplyr::mutate(god_lead1 = as.factor(god_lead1-1))%>%
  dplyr::mutate(spirit_lead1 = as.factor(spirit_lead1-1))%>%
...
out2_god <- mice::as.mids(long3)
saveh(out2_god, "out2_god")
out2_god <- readh("out2_god")
...
m1_god_k6_nb <- brm_multiple(
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m1_god_k6_nb"),
  set_prior('normal(0, 1)', class = 'b')
)

## posterior predictive checks
pp_check(m1_god_k6_nb)

#table
lazerhawk::brms_SummaryTable(m1_god_k6_nb, panderize = F)

# another table
m1_bayes_table <-
  parameters::model_parameters(m1_god_k6_nb)
m1_bayes_table
plot(m1_bayes_table)

# graph
plot_smooth_m1g <- plot(
  conditional_effects(
    m1_god_k6_nb,
    "god_lead1",
    ndraws = 500,
...
m1_plot <-
  ...
  labs(title = "Kessler 6 Distress", # (negative binomial regression)
       ...
       x = "Belief in God") #+ scale_colour_okabe_ito(alpha=.5)

m1_plot

# Try with standardize
m1_god_k6 <- brm_multiple(
  KESSLER6sum_lead2_z ~
    god_lead1 + 
    Believe.God_z +
    AGREEABLENESS_z +
...
#table
lazerhawk::brms_SummaryTable(m1_god_k6, panderize = F)

# another table
m1_bayes_table <-
  parameters::model_parameters(m1_god_k6)
m1_bayes_table
plot(m1_bayes_table)
...
# depressed ---------------------------------------------------------------
m1_ab_god_depressed  <- brm_multiple(
  Depressed_lead2 ~
    god_lead1 + 
    Believe.God_z +
    AGREEABLENESS_z +
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m1_ab_god_depressed"),
  set_prior('normal(0, 1)', class = 'b')
)
...
m2_god_lifemeaning <- brm_multiple(
  LIFEMEANING_lead2_z ~
    god_lead1 + 
    Believe.God_z +
    AGREEABLENESS_z +
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m2_god_lifemeaning"),
  set_prior('normal(0, 1)', class = 'b')
)
...
m2a_god_lifemeaning <- brm_multiple(
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m2a_god_lifemeaning"),
  set_prior('normal(0, 1)', class = 'b')
)


# wow nice
pp_check(m2a_god_lifemeaning)


#table
lazerhawk::brms_SummaryTable(m2a_god_lifemeaning, panderize = F)

# another table
m1_bayes_table <-
  parameters::model_parameters(m2a_god_lifemeaning)
m1_bayes_table
plot(m1_bayes_table)
...
m15a_church_charity <- brm_multiple(
  bf(CharityDonate_lead2 ~
       ...
       SUPPORT_z +#
       Urban_z +
       Volunteers_z, zi ~  god_lead1 + Religion.Believe + CharityDonate_log_z),
...
m16_church_alcohol_intensity <-
  brm_multiple(
    bf(Alcohol.Intensity_lead2   ~
         ...
         SUPPORT_z +#
         Urban_z +
         Volunteers_z, zi ~ god_lead1 + Religion.Believe + Alcohol.Intensity_log_z),
    family = "zero_inflated_negbinomial",
    data = out2_god,
    seed = 1234,
    warmup = 1000,
...
m1_spirit_k6_nb <- brm_multiple(
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m1_spirit_k6_nb"),
  set_prior('normal(0, 1)', class = 'b')
)

## posterior predictive checks
pp_check(m1_spirit_k6_nb)

#table
lazerhawk::brms_SummaryTable(m1_spirit_k6_nb, panderize = F)

# another table
m1_bayes_table <-
  parameters::model_parameters(m1_spirit_k6_nb)
m1_bayes_table
plot(m1_bayes_table)
...
m1_spirit_k6 <- brm_multiple(
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m1_spirit_k6"),
  set_prior('normal(0, 1)', class = 'b')
)

## posterior predictive checks
pp_check(m1_spirit_k6)

#table
lazerhawk::brms_SummaryTable(m1_spirit_k6, panderize = F)
...
# depressed ---------------------------------------------------------------
m1_ab_spirit_depressed  <- brm_multiple(
  Depressed_lead2 ~
    spirit_lead1 + 
    Believe.Spirit_z +
    AGREEABLENESS_z +
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m1_ab_spirit_depressed"),
  set_prior('normal(0, 1)', class = 'b')
)
...
m2_spirit_lifemeaning <- brm_multiple(
  LIFEMEANING_lead2_z ~
    spirit_lead1 + 
    Believe.Spirit_z +
    AGREEABLENESS_z +
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m2_spirit_lifemeaning"),
  set_prior('normal(0, 1)', class = 'b')
)
...
m2a_spirit_lifemeaning <- brm_multiple(
  LIFEMEANING_lead2ord ~
    spirit_lead1 + 
    Believe.Spirit_z +
    AGREEABLENESS_z +
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m2a_spirit_lifemeaning"),
  set_prior('normal(0, 1)', class = 'b')
)


# wow nice
pp_check(m2a_spirit_lifemeaning)


#table
lazerhawk::brms_SummaryTable(m2a_spirit_lifemeaning, panderize = F)

# another table
m1_bayes_table <-
  parameters::model_parameters(m2a_spirit_lifemeaning)
m1_bayes_table
plot(m1_bayes_table)
