...
# number of ids
N <- length(unique(df_in$Id)) # 28521
N
# inspect data
skim(df_in)
...
library(naniar)
naniar::gg_miss_var(mice_in)
vis_miss(mice_in,
         warn_large_data = FALSE)

# any colinear vars?
mice:::find.collinear(mice_in)
...
# impute
in_mice <- mice::mice(mice_in,  seed = 0, m = 10)

# save
saveh(in_mice, "in_mice")

# read
in_mice <- readh("in_mice")

# checks
outlist2 <-
  row.names(in_mice)[in_mice$outflux < 0.5]
length(outlist2)

# checks
head(in_mice$loggedEvents, 10)
...
i_f <- mice::complete(in_mice, "long", inc = F)
i_l <- mice::complete(in_mice, "long", inc = TRUE)
...
# what is a standard deviation unit of log income?
sd(exp(if3$income_log_lead1 ))
122387.8 + 1

mean(exp(if3$income_log_lead1 ))
...
bmi_st <- as.formula(paste("HLTH.BMI_lead2_z ~ bs(income_log_lead1_z) +",
                           paste(baselinevars,
                                 collapse = "+")))

# # this mice code won't work, alas!
# m1_bmi <- with(i_m, exp = glm(bmi_st))

## for some reason, writing a function works.
fit_bmi = function(formula) {
  with(i_m, glm(as.formula(paste("HLTH.BMI_lead2_z ~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))) )
}
# pooled model
...
# unit level change
comps_m0 <- comparisons(m1_long,  variables = "income_log_lead1_z")
...
# sf-health ---------------------------------------------------------------
sfhealth_st <- as.formula(paste("SFHEALTH_lead2_z~ bs(income_log_lead1_z) +",
                                paste(baselinevars,
                                      collapse = "+")))
## fit
fit_sfhealth = function(formula) {
  with(i_m, glm(as.formula(paste("SFHEALTH_lead2_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
# exercise ---------------------------------------------------------------
Hours.Exercise_st <- as.formula(paste("Hours.Exercise_lead2_log_z~ bs(income_log_lead1_z) +",  paste(baselinevars, collapse = "+")))

## fit for mice to work, don't ask why
fit_Hours.Exercise = function(formula) {
  with(i_m, glm(as.formula(paste("Hours.Exercise_lead2_log_z~ bs(income_log_lead1_z) +",  paste(baselinevars,collapse = "+")))))
}
...
HLTH.SleepHours_st <- as.formula(paste("HLTH.SleepHours_lead2_z ~ bs(income_log_lead1_z) +",  paste(baselinevars, collapse = "+")))

## fit
fit_HLTH.SleepHours = function(formula) {
  with(i_m, glm(as.formula(paste("HLTH.SleepHours_lead2_z~ bs(income_log_lead1_z) +",  paste(baselinevars,collapse = "+")))))
}
# ppoled model
...
Smoker_st <- as.formula(paste("Smoker_lead2 ~ bs(income_log_lead1_z) +",
                              paste(baselinevars,
                                    collapse = "+")))

# fit
fit_Smoker = function(formula) {
  with(i_m, glm(as.formula(paste("Smoker_lead2 ~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
...
# formula
HLTH.Fatigue_st <- as.formula(paste("HLTH.Fatigue_lead2_z~ bs(income_log_lead1_z) +",
                                    paste(baselinevars, collapse = "+")))

# fit
fit_Fatigue = function(formula) {
  with(i_m, glm(as.formula(paste("HLTH.Fatigue_lead2_z ~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
Alcohol.Frequency_st <- as.formula(paste("Alcohol.Frequency_lead2ord_z~ bs(income_log_lead1_z) +",
                                         paste(baselinevars, collapse = "+")))

# fit
fit_Alcohol.Frequency = function(formula) {
  with(i_m, glm(as.formula(paste("Alcohol.Frequency_lead2ord_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
Alcohol.Intensity_st <- as.formula(paste("Alcohol.Intensity_log_lead2_z~ bs(income_log_lead1_z) +",
                                         paste(baselinevars,
                                               collapse = "+")))
# fit
fit_Alcohol.Intensity = function(formula) {
  with(i_m, glm(as.formula(paste("Alcohol.Intensity_log_lead2_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
...
Bodysat_st <- as.formula(paste("Bodysat_lead2_z~ bs(income_log_lead1_z) +",
                               paste(baselinevars,
                                     collapse = "+")))
...
fit_Bodysat_st = function(formula) {
  with(i_m, glm(as.formula(paste("Bodysat_lead2_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
...
# rumination --------------------------------------------------------------
Rumination_st <- as.formula(paste("Rumination_lead2ord_z~ bs(income_log_lead1_z) +",
                                  paste(baselinevars,
                                        collapse = "+")))

# fit
fit_Rumination_st = function(formula) {
  with(i_m, glm(as.formula(paste("Rumination_lead2ord_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
...
# sex satisfaction --------------------------------------------------------
SexualSatisfaction_st <- as.formula(paste("SexualSatisfaction_lead2_z~ bs(income_log_lead1_z) +", paste(baselinevars, collapse = "+")))

## fit
fit_SexualSatisfaction_st = function(formula) {
  with(i_m, glm( as.formula(paste("SexualSatisfaction_lead2_z~ bs(income_log_lead1_z) +", paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
EmotionRegulation1_st <- as.formula(paste("EmotionRegulation1_lead2_z~ bs(income_log_lead1_z) +", paste(baselinevars, collapse = "+")))

## fit
fit_EmotionRegulation1 = function(formula) {
  with(i_m, glm(as.formula(paste("EmotionRegulation1_lead2_z~ bs(income_log_lead1_z) +", paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
# emotional reg 2 ---------------------------------------------------------
EmotionRegulation2_st <- as.formula(paste("EmotionRegulation2_lead2_z~ bs(income_log_lead1_z) +",
                                          paste(baselinevars,
                                                collapse = "+")))
...
fit_EmotionRegulation2 = function(formula) {
  with(i_m, glm(as.formula(paste("EmotionRegulation2_lead2_z~ bs(income_log_lead1_z) +", paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
# emotional reg 3 ---------------------------------------------------------
EmotionRegulation3_st <- as.formula(paste("EmotionRegulation3_lead2_z~ bs(income_log_lead1_z) +",
                                          paste(baselinevars,
                                                collapse = "+")))

## fit
fit_EmotionRegulation3 = function(formula) {
  with(i_m, glm(as.formula(paste("EmotionRegulation3_lead2_z~ bs(income_log_lead1_z) +", paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
KESSLER6sum_st <- as.formula(paste("KESSLER6sum_lead2_z~ bs(income_log_lead1_z) +",
                                   paste(baselinevars,
                                         collapse = "+")))
## fit
fit_KESSLER6sum_st = function(formula) {
  with(i_m, glm(as.formula(paste("KESSLER6sum_lead2_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
POWERDEPENDENCE1_st <- as.formula(paste("POWERDEPENDENCE1_lead2_z~ bs(income_log_lead1_z) +",
                                        paste(baselinevars,
                                              collapse = "+")))
## fit
fit_POWERDEPENDENCE1_st = function(formula) {
  with(i_m, glm(as.formula(paste("POWERDEPENDENCE1_lead2_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
# power dependence 2 ------------------------------------------------------
POWERDEPENDENCE2_st <- as.formula(paste("POWERDEPENDENCE2_lead2_z~ bs(income_log_lead1_z) +",
                                        paste(baselinevars,
                                              collapse = "+")))
## fit
fit_POWERDEPENDENCE2_st = function(formula) {
  with(i_m, glm(as.formula(paste("POWERDEPENDENCE2_lead2_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
PERFECTIONISM_st <- as.formula(paste("PERFECTIONISM_lead2_z~ bs(income_log_lead1_z) +",
                                     paste(baselinevars,
                                           collapse = "+")))

## fit
fit_PERFECTIONISM_st = function(formula) {
  with(i_m, glm( as.formula(paste("PERFECTIONISM_lead2_z~ bs(income_log_lead1_z) +",
                                  paste(baselinevars,
                                        collapse = "+")))))
}
# ppoled model
...
SELF.ESTEEM_st <- as.formula(paste("SELF.ESTEEM_lead2_z~ bs(income_log_lead1_z) +",
                                   paste(baselinevars,
                                         collapse = "+")))
## fit
fit_SELF.ESTEEM_st = function(formula) {
  with(i_m, glm( as.formula(paste("SELF.ESTEEM_lead2_z~ bs(income_log_lead1_z) +",
                                  paste(baselinevars,
                                        collapse = "+")))))
}
# ppoled model
...
# gratitude ---------------------------------------------------------------
GRATITUDE_st <- as.formula(paste("GRATITUDE_lead2_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))
## fit
fit_GRATITUDE_st = function(formula) {
  with(i_m, glm( as.formula(paste("GRATITUDE_lead2_z~ bs(income_log_lead1_z) +",
                                  paste(baselinevars,
                                        collapse = "+")))
  ))
}
...
VENGEFUL.RUMIN_st <- as.formula(paste("VENGEFUL.RUMIN_lead2_z~ bs(income_log_lead1_z) +",
                                      paste(baselinevars,
                                            collapse = "+")))

## fit
fit_VENGEFUL.RUMIN_st = function(formula) {
  with(i_m, glm( as.formula(paste("VENGEFUL.RUMIN_lead2_z~ bs(income_log_lead1_z) +",
                                  paste(baselinevars,
                                        collapse = "+")))
  ))
}
...
# life meaning ------------------------------------------------------------
LIFEMEANING_st <- as.formula(paste("LIFEMEANING_lead2_z~ bs(income_log_lead1_z) +",
                                   paste(baselinevars,
                                         collapse = "+")))
## fit
fit_LIFEMEANING_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("LIFEMEANING_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
HONESTY_HUMILITY_st <- as.formula(paste("HONESTY_HUMILITY_lead2_z~ bs(income_log_lead1_z) +",
                                        paste(baselinevars,
                                              collapse = "+")))
## fit
fit_HONESTY_HUMILITY_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("HONESTY_HUMILITY_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# belonging ---------------------------------------------------------------
BELONG_st <- as.formula(paste("BELONG_lead2_z~ bs(income_log_lead1_z) +",
                              paste(baselinevars,
                                    collapse = "+")))
## fit
fit_BELONG_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("BELONG_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# soc support -------------------------------------------------------------
SUPPORT_st <- as.formula(paste("SUPPORT_lead2_z~ bs(income_log_lead1_z) +",
                               paste(baselinevars,
                                     collapse = "+")))
...
fit_SUPPORT_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("SUPPORT_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# volunteers --------------------------------------------------------------
Volunteers_st <- as.formula(paste("Volunteers_lead2~ bs(income_log_lead1_z) +",
                                  paste(baselinevars,  collapse = "+")))


# fit
fit_Volunteers_st = function(formula) {
  with(i_m, glm(as.formula(paste("Volunteers_lead2~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# pooled model
...
CharityDonate_st <- as.formula(paste("CharityDonate_log_lead2_z~ bs(income_log_lead1_z) +",
                                     paste(baselinevars,
                                           collapse = "+")))
...
fit_CharityDonate_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("CharityDonate_log_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
community_st <- as.formula(paste("community_lead2_z~ bs(income_log_lead1_z) +",
                                 paste(baselinevars,
                                       collapse = "+")))
...
fit_community_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("community_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# national wellbeing ------------------------------------------------------
NWI_st <- as.formula(paste("NWI_lead2_z~ bs(income_log_lead1_z) +",
                           paste(baselinevars,
                                 collapse = "+")))
## fit
fit_NWI_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("NWI_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# imperm group ------------------------------------------------------------
ImpermeabilityGroup_st <- as.formula(paste("ImpermeabilityGroup_lead2_z~ bs(income_log_lead1_z) +",
                                           paste(baselinevars,
                                                 collapse = "+")))
## fit
fit_ImpermeabilityGroup_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("ImpermeabilityGroup_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
Standard.Living_st <- as.formula(paste("Standard.Living_lead2ord_z~ bs(income_log_lead1_z) +",
                                       paste(baselinevars,
                                             collapse = "+")))
...
fit_Standard.Living_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("Standard.Living_lead2ord_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
Your.Future.Security_st <- as.formula(paste("Your.Future.Security_lead2_z~ bs(income_log_lead1_z) +",
                                            paste(baselinevars,
                                                  collapse = "+")))
## fit
fit_Your.Future.Security = function(formula) {
  with(i_m, glm(
    as.formula(paste("Your.Future.Security_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
Your.Health_st <- as.formula(paste("Your.Health_lead2_z~ bs(income_log_lead1_z) +",
                                   paste(baselinevars,
                                         collapse = "+")))
...
fit_Your.Health_st= function(formula) {
  with(i_m, glm(
    as.formula(paste("Your.Health_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
Your.Personal.Relationships_st <- as.formula(paste("Your.Personal.Relationships_lead2ord_z~ bs(income_log_lead1_z) +",  paste(baselinevars,collapse = "+")))

## fit
fit_Your.Personal.Relationships_st= function(formula) {
  with(i_m, glm(
    as.formula(paste("Your.Personal.Relationships_lead2ord_z~ bs(income_log_lead1_z) +",  paste(baselinevars,collapse = "+")))
  ))
}
...
Emp.WorkLifeBalance_st <- as.formula(paste("Emp.WorkLifeBalance_lead2_z~ bs(income_log_lead1_z) +",
                                           paste(baselinevars,
                                                 collapse = "+")))
#fit
fit_Emp.WorkLifeBalance_st= function(formula) {
  with(i_m, glm(
    as.formula(paste("Emp.WorkLifeBalance_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
#PermeabilityIndividual ------------------------------------------------------------
PermeabilityIndividual_st <- as.formula(paste("PermeabilityIndividual_lead2_z~ bs(income_log_lead1_z) +",
                                           paste(baselinevars,
                                                 collapse = "+")))
## fit
fit_PermeabilityIndividual_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("PermeabilityIndividual_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
fit_LIFESAT_st = function(formula) {
  with(i_m, glm(
    as.formula(paste("LIFESAT_lead2_z~ bs(income_log_lead1_z) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
