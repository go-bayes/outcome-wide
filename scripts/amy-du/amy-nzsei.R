...
#  dplyr::mutate(HLTH.Disability_lead1 = lead(Hours.Work, n = 1)) %>%
  dplyr::mutate(NZSEI13_lead1 = lead(NZSEI13, n = 1)) %>%
...
library(naniar)
naniar::gg_miss_var(mice_z)
naniar::vis_miss(mice_z,
         warn_large_data = FALSE)

# any colinear vars?
mice:::find.collinear(mice_z)

# impute
z_mice <- mice::mice(mice_z,  seed = 0, m = 10)
...
# checks
outlist2 <-
  row.names(z_mice)[z_mice$outflux < 0.5]
length(outlist2)

# checks
head(z_mice$loggedEvents, 10)
...
z_f <- mice::complete(z_mice, "long", inc = F)
z_l <- mice::complete(z_mice, "long", inc = TRUE)
...
z_l2 <- z_l %>%
  ...
  dplyr::mutate(NZSEI13_lead1_10 =  as.integer(NZSEI13_lead1/10))%>%
...
# MNMONTONIC MODEL?
hist(z_l2$Hours.Work_lead1_sqrt)
...
# Get data into shape
zf3 <- z_f2 %>% mutate_if(is.matrix, as.vector)
zl3 <- z_l2 %>% mutate_if(is.matrix, as.vector)

z_m <- mice::as.mids(zl3)
...
# set up vars -------------------------------------------------------------
hist(zf3$NZSEI13_lead1_10)
ylim <- c(-.2,.2)
xlab <- "NZSEI 13/10 (1-9)"
df <-  z_m
m = 10
X = "NZSEI13_lead1_10"
x = 1:9
...
mean(zf3$NZSEI13_lead1_10)

# bmi ---------------------------------------------------------------------
#conditional  -- not we are using splines
bmi_st <- as.formula(paste("HLTH.BMI_lead2_z ~ bs(NZSEI13_lead1_10) +",
                           paste(baselinevars,
                                 collapse = "+")))

# # this mice code won't work, alas!
# m1_bmi <- with(z_m, exp = glm(bmi_st))

## for some reason, writing a function works.
fit_bmi = function(formula) {
  with(z_m, glm(as.formula(paste("HLTH.BMI_lead2_z ~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))) )
}
# pooled model
...
# unit level change
compz_m0 <- comparisons(m1_long,  variables = "Standard.Living_lead1")
...
ggplot(compz_m0, aes(comparison)) +
  geom_histogram(bins = 30) +
  facet_wrap(~contrast, scale = "free_x") +
  labs(x = "Distribution of unit-level contrasts")

#comparisons at diff levels
compz_m1 <- comparisons(m1_long, variables = list(Hours.Work_lead1_10 = c(0,3))) %>% tidy()
compz_m1

compz_m2 <- comparisons(m1_long, variables = list(Hours.Work_lead1_10 = c(3,6))) %>% tidy()
compz_m2


# sf-health ---------------------------------------------------------------
sfhealth_st <- as.formula(paste("SFHEALTH_lead2_z~ bs(NZSEI13_lead1_10) +",
                                paste(baselinevars,
                                      collapse = "+")))
## fit
fit_sfhealth = function(formula) {
  with(z_m, glm(as.formula(paste("SFHEALTH_lead2_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
# exercise ---------------------------------------------------------------
Hours.Exercise_st <- as.formula(paste("Hours.Exercise_lead2_log_z~ bs(NZSEI13_lead1_10) +",  paste(baselinevars, collapse = "+")))

## fit for mice to work, don't ask why
fit_Hours.Exercise = function(formula) {
  with(z_m, glm(as.formula(paste("Hours.Exercise_lead2_log_z~ bs(NZSEI13_lead1_10) +",  paste(baselinevars,collapse = "+")))))
}
...
HLTH.SleepHours_st <- as.formula(paste("HLTH.SleepHours_lead2_z ~ bs(NZSEI13_lead1_10) +",  paste(baselinevars, collapse = "+")))

## fit
fit_HLTH.SleepHours = function(formula) {
  with(z_m, glm(as.formula(paste("HLTH.SleepHours_lead2_z~ bs(NZSEI13_lead1_10) +",  paste(baselinevars,collapse = "+")))))
}
# ppoled model
...
Smoker_st <- as.formula(paste("Smoker_lead2 ~ bs(NZSEI13_lead1_10) +",
                              paste(baselinevars,
                                    collapse = "+")))

# fit
fit_Smoker = function(formula) {
  with(z_m, glm(as.formula(paste("Smoker_lead2 ~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
...
# formula
HLTH.Fatigue_st <- as.formula(paste("HLTH.Fatigue_lead2_z~ bs(NZSEI13_lead1_10) +",
                                    paste(baselinevars, collapse = "+")))

# fit
fit_Fatigue = function(formula) {
  with(z_m, glm(as.formula(paste("HLTH.Fatigue_lead2_z ~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
Alcohol.Frequency_st <- as.formula(paste("Alcohol.Frequency_lead2ord_z~ bs(NZSEI13_lead1_10) +",
                                         paste(baselinevars, collapse = "+")))

# fit
fit_Alcohol.Frequency = function(formula) {
  with(z_m, glm(as.formula(paste("Alcohol.Frequency_lead2ord_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
Alcohol.Intensity_st <- as.formula(paste("Alcohol.Intensity_log_lead2_z~ bs(NZSEI13_lead1_10) +",
                                         paste(baselinevars,
                                               collapse = "+")))
# fit
fit_Alcohol.Intensity = function(formula) {
  with(z_m, glm(as.formula(paste("Alcohol.Intensity_log_lead2_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
...
Bodysat_st <- as.formula(paste("Bodysat_lead2_z~ bs(NZSEI13_lead1_10) +",
                               paste(baselinevars,
                                     collapse = "+")))
...
fit_Bodysat_st = function(formula) {
  with(z_m, glm(as.formula(paste("Bodysat_lead2_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
...
# rumination --------------------------------------------------------------
Rumination_st <- as.formula(paste("Rumination_lead2ord_z~ bs(NZSEI13_lead1_10) +",
                                  paste(baselinevars,
                                        collapse = "+")))

# fit
fit_Rumination_st = function(formula) {
  with(z_m, glm(as.formula(paste("Rumination_lead2ord_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
...
# sex satisfaction --------------------------------------------------------
SexualSatisfaction_st <- as.formula(paste("SexualSatisfaction_lead2_z~ bs(NZSEI13_lead1_10) +", paste(baselinevars, collapse = "+")))

## fit
fit_SexualSatisfaction_st = function(formula) {
  with(z_m, glm( as.formula(paste("SexualSatisfaction_lead2_z~ bs(NZSEI13_lead1_10) +", paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
EmotionRegulation1_st <- as.formula(paste("EmotionRegulation1_lead2_z~ bs(NZSEI13_lead1_10) +", paste(baselinevars, collapse = "+")))

## fit
fit_EmotionRegulation1 = function(formula) {
  with(z_m, glm(as.formula(paste("EmotionRegulation1_lead2_z~ bs(NZSEI13_lead1_10) +", paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
# emotional reg 2 ---------------------------------------------------------
EmotionRegulation2_st <- as.formula(paste("EmotionRegulation2_lead2_z~ bs(NZSEI13_lead1_10) +",
                                          paste(baselinevars,
                                                collapse = "+")))
...
fit_EmotionRegulation2 = function(formula) {
  with(z_m, glm(as.formula(paste("EmotionRegulation2_lead2_z~ bs(NZSEI13_lead1_10) +", paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
# emotional reg 3 ---------------------------------------------------------
EmotionRegulation3_st <- as.formula(paste("EmotionRegulation3_lead2_z~ bs(NZSEI13_lead1_10) +",
                                          paste(baselinevars,
                                                collapse = "+")))

## fit
fit_EmotionRegulation3 = function(formula) {
  with(z_m, glm(as.formula(paste("EmotionRegulation3_lead2_z~ bs(NZSEI13_lead1_10) +", paste(baselinevars, collapse = "+")))))
}
# ppoled model
...
KESSLER6sum_st <- as.formula(paste("KESSLER6sum_lead2_z~ bs(NZSEI13_lead1_10) +",
                                   paste(baselinevars,
                                         collapse = "+")))
## fit
fit_KESSLER6sum_st = function(formula) {
  with(z_m, glm(as.formula(paste("KESSLER6sum_lead2_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
POWERDEPENDENCE1_st <- as.formula(paste("POWERDEPENDENCE1_lead2_z~ bs(NZSEI13_lead1_10) +",
                                        paste(baselinevars,
                                              collapse = "+")))
## fit
fit_POWERDEPENDENCE1_st = function(formula) {
  with(z_m, glm(as.formula(paste("POWERDEPENDENCE1_lead2_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
# power dependence 2 ------------------------------------------------------
POWERDEPENDENCE2_st <- as.formula(paste("POWERDEPENDENCE2_lead2_z~ bs(NZSEI13_lead1_10) +",
                                        paste(baselinevars,
                                              collapse = "+")))
## fit
fit_POWERDEPENDENCE2_st = function(formula) {
  with(z_m, glm(as.formula(paste("POWERDEPENDENCE2_lead2_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# ppoled model
...
PERFECTIONISM_st <- as.formula(paste("PERFECTIONISM_lead2_z~ bs(NZSEI13_lead1_10) +",
                                     paste(baselinevars,
                                           collapse = "+")))

## fit
fit_PERFECTIONISM_st = function(formula) {
  with(z_m, glm( as.formula(paste("PERFECTIONISM_lead2_z~ bs(NZSEI13_lead1_10) +",
                                  paste(baselinevars,
                                        collapse = "+")))))
}
# ppoled model
...
SELF.ESTEEM_st <- as.formula(paste("SELF.ESTEEM_lead2_z~ bs(NZSEI13_lead1_10) +",
                                   paste(baselinevars,
                                         collapse = "+")))
## fit
fit_SELF.ESTEEM_st = function(formula) {
  with(z_m, glm( as.formula(paste("SELF.ESTEEM_lead2_z~ bs(NZSEI13_lead1_10) +",
                                  paste(baselinevars,
                                        collapse = "+")))))
}
# ppoled model
...
# gratitude ---------------------------------------------------------------
GRATITUDE_st <- as.formula(paste("GRATITUDE_lead2_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))
## fit
fit_GRATITUDE_st = function(formula) {
  with(z_m, glm( as.formula(paste("GRATITUDE_lead2_z~ bs(NZSEI13_lead1_10) +",
                                  paste(baselinevars,
                                        collapse = "+")))
  ))
}
...
VENGEFUL.RUMIN_st <- as.formula(paste("VENGEFUL.RUMIN_lead2_z~ bs(NZSEI13_lead1_10) +",
                                      paste(baselinevars,
                                            collapse = "+")))

## fit
fit_VENGEFUL.RUMIN_st = function(formula) {
  with(z_m, glm( as.formula(paste("VENGEFUL.RUMIN_lead2_z~ bs(NZSEI13_lead1_10) +",
                                  paste(baselinevars,
                                        collapse = "+")))
  ))
}
...
# life meaning ------------------------------------------------------------
LIFEMEANING_st <- as.formula(paste("LIFEMEANING_lead2_z~ bs(NZSEI13_lead1_10) +",
                                   paste(baselinevars,
                                         collapse = "+")))
## fit
fit_LIFEMEANING_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("LIFEMEANING_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
HONESTY_HUMILITY_st <- as.formula(paste("HONESTY_HUMILITY_lead2_z~ bs(NZSEI13_lead1_10) +",
                                        paste(baselinevars,
                                              collapse = "+")))
## fit
fit_HONESTY_HUMILITY_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("HONESTY_HUMILITY_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# belonging ---------------------------------------------------------------
BELONG_st <- as.formula(paste("BELONG_lead2_z~ bs(NZSEI13_lead1_10) +",
                              paste(baselinevars,
                                    collapse = "+")))
## fit
fit_BELONG_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("BELONG_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# soc support -------------------------------------------------------------
SUPPORT_st <- as.formula(paste("SUPPORT_lead2_z~ bs(NZSEI13_lead1_10) +",
                               paste(baselinevars,
                                     collapse = "+")))
...
fit_SUPPORT_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("SUPPORT_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# volunteers --------------------------------------------------------------
Volunteers_st <- as.formula(paste("Volunteers_lead2~ bs(NZSEI13_lead1_10) +",
                                  paste(baselinevars,  collapse = "+")))


# fit
fit_Volunteers_st = function(formula) {
  with(z_m, glm(as.formula(paste("Volunteers_lead2~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))))
}
# pooled model
...
CharityDonate_st <- as.formula(paste("CharityDonate_log_lead2_z~ bs(NZSEI13_lead1_10) +",
                                     paste(baselinevars,
                                           collapse = "+")))
...
fit_CharityDonate_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("CharityDonate_log_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
community_st <- as.formula(paste("community_lead2_z~ bs(NZSEI13_lead1_10) +",
                                 paste(baselinevars,
                                       collapse = "+")))
...
fit_community_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("community_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# national wellbeing ------------------------------------------------------
NWI_st <- as.formula(paste("NWI_lead2_z~ bs(NZSEI13_lead1_10) +",
                           paste(baselinevars,
                                 collapse = "+")))
## fit
fit_NWI_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("NWI_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
# imperm group ------------------------------------------------------------
ImpermeabilityGroup_st <- as.formula(paste("ImpermeabilityGroup_lead2_z~ bs(NZSEI13_lead1_10) +",
                                           paste(baselinevars,
                                                 collapse = "+")))
## fit
fit_ImpermeabilityGroup_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("ImpermeabilityGroup_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
Standard.Living_st <- as.formula(paste("Standard.Living_lead2ord_z~ bs(NZSEI13_lead1_10) +",
                                       paste(baselinevars,
                                             collapse = "+")))
...
fit_Standard.Living_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("Standard.Living_lead2ord_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
Your.Future.Security_st <- as.formula(paste("Your.Future.Security_lead2_z~ bs(NZSEI13_lead1_10) +",
                                            paste(baselinevars,
                                                  collapse = "+")))
## fit
fit_Your.Future.Security = function(formula) {
  with(z_m, glm(
    as.formula(paste("Your.Future.Security_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
Your.Health_st <- as.formula(paste("Your.Health_lead2_z~ bs(NZSEI13_lead1_10) +",
                                   paste(baselinevars,
                                         collapse = "+")))
...
fit_Your.Health_st= function(formula) {
  with(z_m, glm(
    as.formula(paste("Your.Health_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
Your.Personal.Relationships_st <- as.formula(paste("Your.Personal.Relationships_lead2ord_z~ bs(NZSEI13_lead1_10) +",  paste(baselinevars,collapse = "+")))

## fit
fit_Your.Personal.Relationships_st= function(formula) {
  with(z_m, glm(
    as.formula(paste("Your.Personal.Relationships_lead2ord_z~ bs(NZSEI13_lead1_10) +",  paste(baselinevars,collapse = "+")))
  ))
}
...
out_cr <- pool_stglm_contrast(m34_Your.Personal.Relationships, df = df, m = m,  X = X, x = x, r= r)
ggplot_stglm_contrast(out_cr, ylim, main, xlab, ylab)

# Work-life balance -------------------------------------------------------

Emp.WorkLifeBalance_st <- as.formula(paste("Emp.WorkLifeBalance_lead2_z~ bs(NZSEI13_lead1_10) +",
                                           paste(baselinevars,
                                                 collapse = "+")))
#fit
fit_Emp.WorkLifeBalance_st= function(formula) {
  with(z_m, glm(
    as.formula(paste("Emp.WorkLifeBalance_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
ggplot_stglm_contrast(out_cr, ylim = c(-.6,.6), main, xlab, ylab)



#PermeabilityIndividual ------------------------------------------------------------
PermeabilityIndividual_st <- as.formula(paste("PermeabilityIndividual_lead2_z~ bs(NZSEI13_lead1_10) +",
                                              paste(baselinevars,
                                                    collapse = "+")))
## fit
fit_PermeabilityIndividual_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("PermeabilityIndividual_lead2_z~ bs(NZSEI13_lead1_10) +",
                     paste(baselinevars,
                           collapse = "+")))
  ))
}
...
fit_LIFESAT_st = function(formula) {
  with(z_m, glm(
    as.formula(paste("LIFESAT_lead2_z~ bs(NZSEI13_lead1_10) +",
                                   paste(baselinevars,
                                         collapse = "+")))
  ))
}
