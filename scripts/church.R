...
c_df <- df %>%
  ...
  dplyr::filter(!is.na(Church)) %>%    # needed for the intervention!is.na(Church_lead1)) %>%  #needed for the intervention
...
#table
lazerhawk::brms_SummaryTable(m1_church, panderize = F)

# another table
m1_bayes_table <-
  parameters::model_parameters(m1_church)
m1_bayes_table
plot(m1_bayes_table)
...
m1_church_nb <- brm_multiple(
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m1_church_nb")#,
  # set_prior('normal(0, 1)', class = 'b')
)

## posterior predictive checks
pp_check(m1_church_nb)

#table
lazerhawk::brms_SummaryTable(m1_church_nb, panderize = F)
...
# depressed ---------------------------------------------------------------
rm(m1_a_church_depressed)
m1_a_church_depressed  <- brm_multiple(
  Depressed ~
    Church_lead1 +
    Church +
    KESSLER6sum_z +
  ...
  chains = 4,
  backend = "cmdstanr",
  file = here::here("mods", "m1_a_church_depressed"),
  set_prior('normal(0, 1)', class = 'b')
)
m1_a_church_depressed
m1_a_church_depressed <-
  parameters::model_parameters(m1_a_church_depressed)
plot(m1_a_church_depressed)
summary(m1_a_church_depressed)
parameters::model_parameters(m1_a_church_depressed) %>% print_html()
...
#table
lazerhawk::brms_SummaryTable(m5_church_rumination, panderize = F)
...
#table
lazerhawk::brms_SummaryTable(m9_church_support, panderize = F)
...
## posterior predictive checks
pp_check(m15_church_charity)

#table
lazerhawk::brms_SummaryTable(m15_church_charity, panderize = F)

# another table
m1_bayes_table <-
  parameters::model_parameters(m15_church_charity)
m1_bayes_table
plot(m1_bayes_table)
