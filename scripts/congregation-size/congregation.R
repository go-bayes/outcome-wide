...
table(
  cc3$Religion.CongregationSize_dunbar1 == cc3$Religion.CongregationSize_lead1_dunbar1
)
...
baselinevars = c(
  ...
  "Relid_z",
  "Respect.Self_z",
  "Religion.CongregationSize_log_z",
  "RWA_z",
  "SDO_z",
  "SELF.CONTROL_z",
  "SELF.ESTEEM_z",
...
# no changes
table(
  cc3$Religion.CongregationSize_lead1_lowerdunbar == cc3$Religion.CongregationSize_lowerdunbar
) / 10
...
fit_HONESTY_HUMILITY_st = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "HONESTY_HUMILITY_lead2_z~ bs(Religion.CongregationSize_lead1_log)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
fit_NZSEI06_st = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "NZSEI06_lead2_10 ~ bs(Religion.CongregationSize_lead1_log)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
fit = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "Church_lead2 ~ bs(Religion.CongregationSize_lead1_log)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
fit_0 = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "SFHEALTH_lead2_z~ bs(SUPPORT_lead1_z)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
fit_0 = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "HLTH.Fatigue_lead2_z~ bs(SUPPORT_lead1_z)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
fit_0 = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "KESSLER6sum_lead2_z~ bs(SUPPORT_lead1_z)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
fit_0 = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "PWI_lead2_z~ bs(SUPPORT_lead1_z)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
fit_0 = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "LIFESAT_lead2_z~ bs(SUPPORT_lead1_z)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
fit_0 = function(formula) {
  with(ccu, glm(as.formula(
    paste(
      "BELONG_lead2_z~ bs(SUPPORT_lead1_z)+",
      paste(baselinevars,
            collapse = "+")
    )
  )))
...
out <-
  with(ctrim_st, glm(SUPPORT_lead2_z ~ bs(Religion.CongregationSize_lead1_log) +))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
...
out <-
  with(ctrim_st, glm(Volunteers_lead2 ~ bs(Religion.CongregationSize_lead1_log) +))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
...
out <-
  with(ctrim_st, glm(
    CharityDonate_lead2 ~ bs(Religion.CongregationSize_lead1_log) + ,
    family = "poisson"
  ))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
...
out <-
  with(ctrim_st, glm(community_lead2_z ~ bs(Religion.CongregationSize_lead1_log) +))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
...
out <-
  with(ctrim_st, glm(NWI_lead2_z ~ bs(Religion.CongregationSize_lead1_log) +))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
...
out <-
  with(ctrim_st, glm(
    ImpermeabilityGroup_lead2 ~ bs(Religion.CongregationSize_lead1_log) +
  ))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
...
out <-
  with(ctrim_st, glm(
    Standard.Living_lead2ord_z ~ bs(Religion.CongregationSize_lead1_log) +
  ))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
...
out <-
  with(ctrim_st, glm(
    Your.Personal.Relationships_lead2ord_z ~ bs(Religion.CongregationSize_lead1_log) +
  ))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
