...
#qp <- quickpred(a_mice) # https://stefvanbuuren.name/fimd/sec-toomany.html
# qp
#for_mice$inc_prop <-
...
long2 <- long %>%
 ...
 # rowwise |>
 # dplyr::mutate(LIFEMEANING = mean(c(LifeMeaning01, LifeMeaning02, na.rm=TRUE))) %>%
  dplyr::mutate(across(where(is.numeric), ~ scale(.x), .names = "{col}_z")) %>%
  select(-c(.imp_z, .id_z))# Respect for Self is fully missing

  table(long2$Church_lead1bin)
  table(long2$LIFEMEANING)
# review
...
out <- with(ctrim, glm( HLTH.SleepHours ~ Church_lead1 ,
                        family = "poisson"))
output <- pool(out, dfcom = NULL)
summary(output, conf.int = TRUE)
exp( +  )
