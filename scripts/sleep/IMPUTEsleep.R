...
# 2018/ 2019 - sleep above 3 and below 13
...
dat_prep  <- dat_new %>%
  ...
  dplyr::mutate(across(c(HLTH.SleepHours),
                       ~ lead(.x, n = 1),
  ...
  dplyr::filter(HLTH.SleepHours > 3 &
                  HLTH.SleepHours  < 13) %>% # min income
  dplyr::filter(HLTH.SleepHours_lead1 > 3 &
                  HLTH.SleepHours < 13) %>% # min income
  dplyr::select(-c(
    Religion.Church,
    Respect.Self_lead2,
...
skim(dat_prep) |>
  arrange(n_missing)
# save data
saveh(dat_prep, "outcomewide-sleep-dat_prep")

# read if needed
dat_prep <- readh("outcomewide-sleep-dat_prep")
...
# save
saveh(ppm_mice, "outcomewide-sleep-ppm_mice")

# read
ppm_mice <- readh("outcomewide-sleep-ppm_mice")
...
ml <- ml %>%
  ...
  dplyr::group_by(id) |> mutate(PWI_lead2 = mean(
    ...
    na.rm = TRUE
  )) |> mutate(POWERDEPENDENCE = mean(c(POWERDEPENDENCE1,
                                        POWERDEPENDENCE2),
                                      na.rm = TRUE)) |>
  mutate(POWERDEPENDENCE_lead2 = mean(c(
...
#save
saveh(ml, "outcomewide-sleep-ml")
saveh(mf, "outcomewide-sleep-mf")
...
saveh(no_miss_model, "outcomewide-sleep-no-miss-model")
